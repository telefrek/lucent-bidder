        ...
        public static string ToImageLinkMarkup(this BidContext context, Uri baseUri)
        {
            if (context.Content.ContentType == ContentType.Banner)
            {
                return @"<a href=""{0}""><img src=""{1}"" width=""{2}"" height=""{3}""></a>".FormatWith(
                    new Uri(baseUri, "/v1/postback?" + QueryParameters.LUCENT_BID_CONTEXT_PARAMETER + "=" + context.ToString() + "&" + QueryParameters.LUCENT_REDIRECT_PARAMETER + "=" + context.FormatLandingPage().SafeBase64Encode()).AbsoluteUri,

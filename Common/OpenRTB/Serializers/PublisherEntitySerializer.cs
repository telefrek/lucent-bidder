...
namespace Lucent.Common.OpenRTB.Serializers
{
    ...
    /// 
    /// </summary>
    public sealed class PublisherEntitySerializer : IEntitySerializer<Publisher>
    {
        /// <inheritdoc />
        public Publisher Read(ISerializationStreamReader serializationStreamReader)
            => ReadAsync(serializationStreamReader, CancellationToken.None).Result;

        /// <inheritdoc />
        public async Task<Publisher> ReadAsync(ISerializationStreamReader serializationStreamReader, CancellationToken token)
        {
        ...
        /// <inheritdoc />
        public void Write(ISerializationStreamWriter serializationStreamWriter, Publisher instance)
            => WriteAsync(serializationStreamWriter, instance, CancellationToken.None).Wait();

        /// <inheritdoc />
        public async Task WriteAsync(ISerializationStreamWriter serializationStreamWriter, Publisher instance, CancellationToken token)
        {
            if (instance != null)
            {
